...
with(pdata, plot(Timestamp, Global_active_power, type="l", xlab="", ylab="Global Active Power"))
with(pdata, plot(Timestamp, Voltage, type="l", xlab="datetime", ylab="Voltage"))
ymax <- max(pdata$Sub_metering_1)
with(pdata, {
  plot(Timestamp, Sub_metering_1, type="n", xlab="", ylim=c(0,ymax), ylab="Energy sub metering")
  par(new=TRUE)
  plot(Timestamp, Sub_metering_1, type="l", ylim=c(0,ymax), axes=FALSE, col="black", ylab="")
  par(new=TRUE)
  plot(Timestamp, Sub_metering_2, type="l", ylim=c(0,ymax), axes=FALSE, col="red", ylab="")
  par(new=TRUE)
  plot(Timestamp, Sub_metering_3, type="l", ylim=c(0,ymax), axes=FALSE, col="blue", ylab="")
  legend("topright", lty = 1, bty="n", col = c("grey", "blue", "red"), legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))
})
with(pdata, plot(Timestamp, Global_reactive_power, type="l", xlab="datetime"))
dev.off()